...
class MainWindow:
	...
	def import_spacer_line_visible(self) -> bool:
		...
		if self.start_scenario_button_visible() or ui.layouts.continue_scenario_button_visible():
	...
	def main_menu_layout(self) -> list:
		...
		[sg.Button("Start New Scenario", key = "Start New Scenario Button", visible = self.start_scenario_button_visible())],
		[sg.Button("Continue Scenario", key = "Continue Scenario Button", visible = ui.layouts.continue_scenario_button_visible())],
		[sg.Text("", key = "Import Spacer Line", visible = self.import_spacer_line_visible())],
		[sg.Button("Import New PDF", key = "Import New PDF Button")]
		]
		
		return layout
	
	def build(self):
		main_menu_column = sg.Column(self.main_menu_layout(), key = "Main Menu Column")
		extract_pdf_column = sg.Column(ui.layouts.extract_pdf_layout(), key = "Extract PDF Column", visible = False)
		select_season_column = sg.Column(ui.layouts.select_season_layout(), key = "Select Season Column", visible = False)
		select_scenario_column = sg.Column(ui.layouts.select_scenario_layout(), key = "Select Scenario Column", visible = False)
		
		layout = [
			[main_menu_column, extract_pdf_column, select_season_column, select_scenario_column]
		]
	
		self.window = sg.Window("Pathfinder Society Assistant", layout, location = self.location, size = self.size, element_padding = 2, margins = (10, 10), resizable = True)
	...
	def update_select_season_column(self):
		...
		for season in season_list:
			self.window.extend_layout(self.window["Select Season Column"], [[sg.Button(f"Season {season}", key = f"Season Selected Button {season}")]])
			self.window.refresh()
	...
	def update_select_scenario_column(self, idx):
		...
		for scenario in scenario_list:
			self.window.extend_layout(self.window["Select Scenario Column"], [[sg.Button(f"Scenario {scenario}", key = f"Scenario Selected Button {scenario}", metadata = idx)]])
